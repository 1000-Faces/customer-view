...
<div class="d-flex my-4">
  <button
    id="push"
    class="mr-4 inline-block rounded border-2 border-blue-600 px-6 py-2.5 text-xs font-medium uppercase leading-tight text-blue-600 shadow-md transition duration-150 ease-in-out hover:shadow-lg focus:shadow-lg focus:outline-none focus:ring-0 active:shadow-lg">
    Push
  </button>
  <button
    id="side"
...
interface SidenavInstance {
  changeMode(mode: string): void;
}

initTE({ Sidenav });

const instanceMode: SidenavInstance = Sidenav.getInstance(
  document.getElementById("sidenav-2")
);
const modes: string[] = ["push", "over", "side"];

modes.forEach((mode: string) => {
  const modeSwitch = document.getElementById(mode);
  modeSwitch.addEventListener("click", () => {
    const instance: SidenavInstance = Sidenav.getInstance(
      document.getElementById("sidenav-2")
    );
    instance.changeMode(mode);
    modes.forEach((el: string) => {
      if (el === mode) {
        ["text-blue-600", "border-blue-600"].forEach((item: string) =>
          modeSwitch.classList.remove(item)
        );
        modeSwitch.className +=
          " bg-blue-600 text-white hover:bg-blue-700 active:bg-blue-800 focus:bg-blue-700 border-transparent";
      } else {
        const node: HTMLElement = document.getElementById(el);
        ...
        [
          ...
          "focus:bg-blue-700",
          "border-transparent",
        ].forEach((item: string) => node.classList.remove(item));
      }
    });
  });
});
